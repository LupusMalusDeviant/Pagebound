...
// Cross-Reference-Streams werden mit klarer Meldung abgelehnt. Zertifikat und
// Passwort werden ausschließlich lokal im Browser verarbeitet — nichts
// verlässt das Gerät.
//
...
function issuerAndSerial(certAsn1: forge.asn1.Asn1): { issuer: forge.asn1.Asn1; serial: forge.asn1.Asn1 } {
  ...
  if (!serial || !issuer) {
    throw new Error("Das Signatur-Zertifikat hat keine erwartbare Struktur (Seriennummer/Aussteller nicht gefunden).");
  }
...
function findPreviousStartXref(bytes: Uint8Array): number {
  ...
  if (idx < 0) throw new Error("Die PDF hat kein abschließendes 'startxref' — inkrementelles Signieren nicht möglich.");
  const m = /startxref\s+(\d+)/.exec(tail.slice(idx));
  if (!m) throw new Error("Der 'startxref'-Eintrag der PDF ist unlesbar.");
  return Number(m[1]);
}
...
function appendIncrementalUpdate(original: Uint8Array, ctx: PDFDocument["context"], touched: PDFRef[]): Uint8Array {
  ...
  if (!/^\s*xref\b/.test(atPrev)) {
    throw new Error(
      "Die PDF nutzt Cross-Reference-Streams (PDF 1.5+). Inkrementelles Signieren ist dafür nicht implementiert — " +"bitte das Dokument zuvor mit klassischer xref-Struktur speichern (jedes von Pagebound erzeugte PDF hat sie).",
    );
  }
...
export async function signPdf(
  ...
  // Bereits signiert? Dann wird angehängt statt neu geschrieben.
  ...
  if (!acroForm) {
    ...
    touched.push(ctx.trailerInfo.Root);
  }
