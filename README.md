...
- **PDF → PDF/A (Best Effort)** — XMP (pdfaid 2/B), sRGB-OutputIntent, Bereinigung (OpenAction/JS), optionales Flatten, **Standard-Schriften werden per Liberation-Fonts eingebettet** (OFL, self-hosted); Rest-Fälle werden ehrlich als Warnung gemeldet (keine Konformitätsgarantie — extern z. B. mit veraPDF prüfen)
...
- **MCP-Server für LLM-Agenten** — Pagebounds PDF-Operationen **tokenlos**, lokal über **stdio** oder **gehostet** über **Streamable HTTP** (`…/mcp`); **29 Tools** (Seiten-Werkzeuge, Split, Stempeln, AES-256-Verschlüsselung, AcroForm-Formulare lesen/ausfüllen/anlegen, Metadaten, **PDF/A-2b und -3b Best-Effort** (inkl. Liberation-Font-Einbettung und eingebetteter Dateien für **E-Rechnung/ZUGFeRD**), **PDF/UA-Kennzeichnung**, **zertifikatsbasierte Signatur (P12, PAdES-B-B)**, Text, Text-Diff/Vergleich, Tabellen→CSV, Bilder→PDF, **Designer**: Designs erzeugen/validieren/mit JSON-Daten füllen/als HTML **oder direkt als PDF** rendern im PWA-kompatiblen `*.pbdesign.json`-Format), base64-I/O mit Größen-/Seiten-Limits; **reproduzierbar** (gleiche Eingabe → byte-gleiches PDF) mit stabilen Fehlerkennungen
...
- Echtes Struktur-Tagging (PDF/UA) für Bestands-PDFs; PAdES-B-T (Zeitstempel) für die Zertifikat-Signatur
