...
- **MCP-Server für LLM-Agenten** — Pagebounds PDF-Operationen **tokenlos**, lokal über **stdio** oder **gehostet** über **Streamable HTTP** (`…/mcp`); **24 Tools** (Seiten-Werkzeuge, Split, Stempeln, AES-256-Verschlüsselung, AcroForm-Formulare lesen/ausfüllen/anlegen, Metadaten, **PDF/A Best-Effort** (inkl. Liberation-Font-Einbettung), **PDF/UA-Kennzeichnung**, **zertifikatsbasierte Signatur (P12)**, Text, Text-Diff/Vergleich, Tabellen→CSV, Bilder→PDF, **Designer**: Designs erzeugen/validieren/als HTML rendern im PWA-kompatiblen `*.pbdesign.json`-Format), base64-I/O mit Größen-/Seiten-Limits
